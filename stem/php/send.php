<?php

use PHPMailer\PHPMailer\PHPMailer;
use PHPMailer\PHPMailer\SMTP;
use PHPMailer\PHPMailer\Exception;

//Load Composer's autoloader
require '../vendor/autoload.php';
    $from="info@atc.com.ng";
    $password="infoinfo"; //stemstem,academy,infoinfo,workstation
    $support = 'support@atc.com.ng';
    $CcEmail=['bbm@atc.com.ng','solihadio@atc.com.ng','john.adebayo@atc.com.ng'];
    $email= "ope@kibo.school";
    $mailSubject ='Letter of Appreciation - Greet & Meet Session';
    $payer="";
    $type=explode('@',$from)[0];
    $type=strtoupper($type);
    
if (isset($email)) {
    $error = "";
    $mail = new PHPMailer(true);
    $mail->IsSMTP();
    $mail->Host = "mail.atc.com.ng";
    $mail->SMTPAuth = true;
    $mail->SMTPSecure = "ssl";
    $mail->Username = $from;
    $mail->Password = $password;
    $mail->Port = "465";
    
    try {
        $mail->setFrom($from, "ATC HUB");//${type}");
        $mail->addAddress($email);
        $mail->addReplyTo($from, "ATC ${type}");
        foreach($CcEmail as $key=>$value){
       $mail->addCC($value);            
        }
        $mail->addCC($support);

       // $mail->addAttachment($filename, 'Payment Invoice');    //Optional name

        //Content
        $mail->isHTML(true); //Set email format to HTML
        $mail->Subject = $mailSubject;
        $variables = ['${header}', '${payer}','${guardName}']; // placeholder in template
        $values = ["{$mailSubject}!",$payer, "Ope Bukola"];
        $mailMessage = file_get_contents("tes.html");
        $mailMessage = str_ireplace($variables, $values, $mailMessage);

        $mail->Body    = $mailMessage;
        $mail->AltBody = 'Kindly find your payment invoice attached';

        $mail->send();
        $message = ['message' => "successfully sent"];
      } catch (Exception $e) {
        $message = ["message" => "Sorry, we are presently unable to send the login details \n
    Mailer Error: {$mail->ErrorInfo}"];
    }
}
var_dump($message['message']);
?>